...
$$
\begin{aligned}&\text{状态空间 }S\text{ 为棋盘的当前状态}\\&\text{动作空间 }A\text{ 为落子的位置}\\&\text{初始状态 }s_0\text{ 为初始状态}\\&\text{目标测试 }GOAL:S\to\{True,False\}\text{ 为胜利}\\&\text{路径耗散 }c_{ij}\text{ 始终为1}\\&\text{找到从初始状态 }s_0\text{ 出发到使得目标测试 }GOAL(s)\text{ 为真的状态 }s\text{ 的一条路径。}\end{aligned}
$$
